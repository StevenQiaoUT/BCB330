...
def create_composite_tissue_grid(svg_files_dict, output_file, cols=4, cell_width=400, cell_height=300, spacing=50):
    ...
    - Columns: 4 timepoints (D0, R15, W0, W15)
    ...
    cell_type_positions = {
        'epidermal': {
            'guard': {'x': 0, 'scale': 0.8},
            'epidermal': {'x': 130, 'scale': 0.8},
            'trichome': {'x': 260, 'scale': 0.8}
        },
        'mesophyll': {
            'palisade': {'x': 50, 'scale': 0.9},
            'spongy': {'x': 220, 'scale': 0.9}
        },
        'vascular': {
            'vascular': {'x': 100, 'scale': 0.6}  # Centered, scaled down
        }
    }
    ...
    for row_idx, tissue_category in enumerate(tissue_categories):
        for col_idx, timepoint in enumerate(timepoints):
            ...
            svg_content.append(
                ...
                f'fill="none" stroke="#cccccc" stroke-width="1" stroke-dasharray="5,5"/>'
            )
